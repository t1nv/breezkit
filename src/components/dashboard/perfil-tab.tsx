...
export function PerfilTab({
  ...
  // Re-sync local edits when fresh server data arrives (guarded render update,
  // see react.dev "storing information from previous renders").
  const [prevFin, setPrevFin] = useState(fin);
  const [prevName, setPrevName] = useState(displayName);
  if (prevFin !== fin) {
    setPrevFin(fin);
    setForm(fin);
  }
