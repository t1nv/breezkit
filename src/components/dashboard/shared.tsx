...
export const inputCls =
  "w-full border-2 border-input bg-background px-3 py-2.5 text-sm font-medium focus:outline-none focus:ring-0";
  ...
  return (
    <span className="relative inline-flex">
      ...
      {open && (
        <span
          role="tooltip"
          className="absolute z-50 left-1/2 -translate-x-1/2 top-full mt-1.5 w-56 bg-popover text-popover-foreground border-2 border-border shadow-[5px_5px_0_var(--foreground)] px-3 py-2 text-xs leading-relaxed"
        >
          {text}
        </span>
      )}
...
export function AIInsightCard({
  ...
  return (
    <div className="border-2 border-border bg-primary p-4 shadow-[6px_6px_0_var(--foreground)]">
      <div className="flex items-center gap-2 mb-1.5">
        <span className="text-xs font-black text-primary-foreground uppercase tracking-[0.18em]">
          {label}
        </span>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        ...
        {remaining > 0 && (
          <motion.button
            ...
            onClick={() => {
              onAdd(remaining);
              confetti({ particleCount: 80, spread: 60, origin: { y: 0.5 }, colors: ["#FF5A00", "#FF8C00", "#FF3D00", "#ffffff"] });
            }}
            disabled={pending}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
...
export function TxEditRow({
  ...
  today: string;
  onCancel: () => void;
  onSave: (patch: { amount: number; category: string; description: string | null; occurred_on: string }) => void;
  pending: boolean;
}) {
