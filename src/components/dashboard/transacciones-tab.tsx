...
export function TransaccionesTab({ userId, txs }: { userId: string; txs: Tx[] }) {
  ...
  const changeType = (next: "income" | "expense") => {
    setType(next);
    setCategory(next === "income" ? "Salario" : "Comida");
  };
  ...
  const add = useMutation({
    mutationFn: async () => {
      ...
      if (desc.length < 3) throw new Error("Agregá una descripción de al menos 3 caracteres.");
        ...
        <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="text-xs rounded-xl border border-border bg-background hover:bg-muted px-3 py-1.5 transition disabled:opacity-40 inline-flex items-center gap-1.5 font-bold"
            >
